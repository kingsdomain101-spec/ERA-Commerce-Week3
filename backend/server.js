...
app.post('/users', async (req, res) => {
  ...
  try {
    ...
    db.query(sql, [first_name, last_name, email, hashPassword], (err, result) => {
      if (err) {
...
app.get("/orders/:id", authenticateToken, (req, res) => {
  const {id} = req.params;
  const sql = "SELECT o.id, o.status, o.total_amount, o.created_at, oi.id AS item_id, oi.quantity, oi.price_at_purchase, oi.subtotal, p.name AS product_name FROM orders o INNER JOIN order_items oi ON o.id = oi.order_id INNER JOIN products p ON p.id = oi.product_id WHERE o.id = ? ORDER BY oi.id ASC";
  ...
  db.beginTransaction(async (err) => {
    ...
    try {
      ...
      const orderResult = await new Promise((resolve, reject) => {
        db.query(orderSql, [userId, totalAmount], (err, results) => {
...
//Get /reports/top-products - admin only data
...
//Get /reports/category-sales - admin only data

app.get("/reports/category-sales", authenticateToken, authorizeRole("admin"), (req, res) => {
  const sql = "SELECT c.id, c.name AS category_name, count(oi.id) AS item_sold, SUM(oi.subtotal) AS total_revenue, AVG(oi.subtotal) AS average_item_value FROM order_items oi INNER JOIN products p ON p.id = oi.product_id RIGHT JOIN categories c ON c.id = p.category_id GROUP BY c.id, c.name HAVING SUM(oi.subtotal) > 0 ORDER BY total_revenue DESC";
  db.query(sql, (err, results) => {
    if (err) return res.status(500).json({ message: "server error"});
    res.json(results);
  });
